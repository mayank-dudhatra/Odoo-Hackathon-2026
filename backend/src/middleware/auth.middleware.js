...
async function authenticate(req, res, next) {
  try {
    ...
    const cacheKey = `auth:${payload.sub}:${payload.sid || "nosid"}`;
    let authUser = authCache.get(cacheKey);
    ...
    if (!authUser) {
      ...
      // If user does not yet have employee_id assigned, resolve or auto-provision once
      if (!user.employee_id) {
      ...
      authCache.set(cacheKey, authUser, 60);
    }
